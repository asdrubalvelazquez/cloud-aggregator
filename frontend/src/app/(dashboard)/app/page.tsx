      ...
      <div className="w-full max-w-6xl space-y-6">
        ...
        {!loading && !hardError && data && (
          <>
            ...
            {billingQuota && (
              <section className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 border border-slate-700 shadow-lg">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <h2 className="text-2xl font-bold text-white">Plan & Límites</h2>
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                      billingQuota.plan === 'free' || billingQuota.plan.includes('free')
                        ? 'bg-slate-600 text-slate-200'
                        : billingQuota.plan.includes('standard')
                        ? 'bg-blue-600 text-white'
                        : 'bg-purple-600 text-white'
                    }`}>
                      {billingQuota.plan === 'free' ? 'FREE' : 
                       billingQuota.plan.includes('standard') ? 'STANDARD' : 
                       billingQuota.plan.includes('premium') ? 'PREMIUM' : 
                       billingQuota.plan.toUpperCase()}
                    </span>
                  </div>
                  {(billingQuota.plan === 'free' || billingQuota.plan.includes('free')) && (
                    <a
                      href="/pricing"
                      className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold transition-colors shadow-lg"
                    >
                      <span>⬆️</span>
                      Actualizar Plan
                    </a>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold text-white">
                        {billingQuota.copies.used} / {billingQuota.copies.limit === null ? '∞' : billingQuota.copies.limit}
                      </span>
                    </div>
                    {billingQuota.copies.limit !== null && (
                      <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ 
                            width: `${Math.min((billingQuota.copies.used / billingQuota.copies.limit) * 100, 100)}%` 
                          }}
                        />
                      </div>
                    )}
                    {billingQuota.copies.limit === null && (
                      <p className="text-xs text-emerald-400">Ilimitada ✨</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold text-white">
                        {billingQuota.transfer.used_gb?.toFixed(2)} / {
                          billingQuota.transfer.limit_gb === null 
                            ? '∞' 
                            : `${billingQuota.transfer.limit_gb.toFixed(2)} GB`
                        }
                      </span>
                    </div>
                    {billingQuota.transfer.limit_gb !== null && billingQuota.transfer.limit_gb > 0 && (
                      <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                        <div
                    ...
                    {billingQuota.transfer.limit_gb === null && (
                      <p className="text-xs text-blue-400">Ilimitada ✨</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-slate-400">Máx por archivo</span>
                      <span className="text-sm font-bold text-white">
                        {billingQuota.max_file_gb.toFixed(1)} GB
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 leading-relaxed">
                      {billingQuota.max_file_gb < 10 ? (
                        <>Actualiza a <span className="text-emerald-400 font-medium">Standard</span> para 10 GB</>
                      ) : billingQuota.max_file_gb < 25 ? (
                        <>Actualiza a <span className="text-purple-400 font-medium">Premium</span> para 25 GB</>
                      ) : (
                        <>Tienes el máximo tamaño disponible</>
                      )}
                    </p>
                  </div>
                </div>
